...
func GetConceptDailyThs(indexCode string, kType int, adjustType int, wait time.Duration) ([]ConceptDailyBar, error) {
    ...
    text := resp.String()
    l := strings.Index(text, "{"); if l < 0 { return []ConceptDailyBar{}, nil }
    payload := text[l:]
    var j struct{ Data string `json:"data"` }
    if err := json.Unmarshal([]byte(payload[:len(payload)-1]), &j); err != nil { return nil, err }
    rows := strings.Split(j.Data, ";")
    out := make([]ConceptDailyBar, 0, len(rows))
    for _, r := range rows { if r == "" { continue }
...
func GetConceptMinuteThs(indexCode string, wait time.Duration) ([]ConceptMinuteBar, error) {
    ...
    if err != nil { return nil, err }
    text := resp.String()
    l := strings.Index(text, "{"); if l < 0 { return []ConceptMinuteBar{}, nil }
    ...
    data := obj[key]
    if data == nil { return []ConceptMinuteBar{}, nil }
...
func GetConceptCurrentThs(indexCode string, kType int, wait time.Duration) (ConceptCurrent, error) {
    ...
    text := resp.String()
    l := strings.Index(text, "{"); if l < 0 { return ConceptCurrent{}, nil }
    payload := text[l:len(text)-1]
    var obj map[string]map[string]any
    if err := json.Unmarshal([]byte(payload), &obj); err != nil { return ConceptCurrent{}, err }
    key := fmt.Sprintf("48_%s", indexCode)
    data := obj[key]
    if data == nil { return ConceptCurrent{}, nil }
