...
func (c *Client) ListEast(ctx context.Context, daysType int) ([]ConceptFlow, error) {
	if daysType == 0 {
		daysType = 1
	}
	...
	for p := 1; p <= 50; p++ {
		...
		go func() {
			...
			if err != nil {
				ch <- pageRes{page: page, rows: []ConceptFlow{}}
				return
			}
			text := resp.String()
			if len(text) < 2 || resp.StatusCode() != 200 {
				ch <- pageRes{page: page, rows: []ConceptFlow{}}
				return
			}
			...
			if j.Data.Diff == nil || len(j.Data.Diff) == 0 {
				ch <- pageRes{page: page, rows: []ConceptFlow{}}
				return
			}
	...
	for i := 0; i < 50; i++ {
		res := <-ch
		if len(res.rows) > 0 {
			pages[res.page] = res.rows
		}
	}
	out := make([]ConceptFlow, 0, 2000)
	for p := 1; p <= 50; p++ {
		if rows, ok := pages[p]; ok {
			out = append(out, rows...)
		}
	}
