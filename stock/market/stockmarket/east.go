...
func GetMarketDailyEast(stockCode string, startDate string, endDate string, kType int, adjustType int, wait time.Duration) ([]DailyBar, error) {
    client := httpc.NewClient()
    seCid := "0"
    if strings.HasPrefix(stockCode, "6") { seCid = "1" }
    beg := strings.ReplaceAll(startDate, "-", "")
    if beg == "" { beg = "19900101" }
    end := strings.ReplaceAll(endDate, "-", "")
    if end == "" { end = time.Now().Format("20060102") }
    klt := kType
    if klt < 5 { klt = 100 + klt }
    ...
    var data struct{ Data struct{ Klines []string `json:"klines"`; PreClose float64 `json:"preClose"` } `json:"data"` }
    if err := json.Unmarshal(resp.Body(), &data); err != nil { return nil, err }
    if len(data.Data.Klines) == 0 { return []DailyBar{}, nil }
...
func GetMarketMinuteEast(stockCode string, wait time.Duration) ([]MinuteBar, error) {
    ...
    if err != nil { return nil, err }
    var res struct{ Data struct{ PreClose float64 `json:"preClose"`; Trends []string `json:"trends"` } `json:"data"` }
    if err := json.Unmarshal(resp.Body(), &res); err != nil { return nil, err }
    if res.Data.Trends == nil { return []MinuteBar{}, nil }
    ...
    for _, row := range res.Data.Trends {
        ...
        if res.Data.PreClose != 0 { bar.ChangePct = bar.Change / res.Data.PreClose * 100 }
        out = append(out, bar)
    }
    return out, nil
}
