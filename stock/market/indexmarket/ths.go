...
func mapIndexToTHS(indexCode string) string {
    m, err := cache.LoadIndexCodeRelTHS()
    if err != nil { return indexCode }
    if v, ok := m[indexCode]; ok && v != "" { return v }
    return indexCode
}

func yearsFrom(startDate string) []string {
    if startDate == "" { return []string{time.Now().Format("2006") } }
    s := strings.Split(startDate, "-")
    y := s[0]
    sy, _ := strconv.Atoi(y)
    cy := time.Now().Year()
    out := make([]string, 0, cy-int(sy)+1)
    for yy := int(sy); yy <= cy; yy++ { out = append(out, fmt.Sprintf("%04d", yy)) }
    return out
}
...
func GetIndexDailyThs(indexCode, startDate string, kType int, wait time.Duration) ([]IndexDailyBar, error) {
    ...
    for _, year := range yrs {
        ...
        if err != nil { continue }
        text := resp.String()
        l := strings.Index(text, "{"); if l < 0 { continue }
        payload := text[l:len(text)-1]
        var j struct{ Data string `json:"data"` }
        if err := json.Unmarshal([]byte(payload), &j); err != nil { continue }
    ...
    if len(out) == 0 { return []IndexDailyBar{}, nil }
    sort.Slice(out, func(i, j int) bool { return out[i].TradeDate < out[j].TradeDate })
    for i := range out { if i > 0 {
        out[i].Change = out[i].Close - out[i-1].Close
        if out[i-1].Close != 0 { out[i].ChangePct = out[i].Change / out[i-1].Close * 100 }
    }}
    return out, nil
}
...
func GetIndexMinuteThs(indexCode string, wait time.Duration) ([]IndexMinuteBar, error) {
    ...
    text := resp.String()
    l := strings.Index(text, "{"); if l < 0 { return []IndexMinuteBar{}, nil }
    payload := text[l:len(text)-1]
    var obj map[string]map[string]any
    if err := json.Unmarshal([]byte(payload), &obj); err != nil { return nil, err }
    key := fmt.Sprintf("zs_%s", concept)
    data := obj[key]
    if data == nil { return []IndexMinuteBar{}, nil }
    ...
    for _, r := range lines { if r == "" { continue }
        cols := strings.Split(r, ",")
        ...
        if pre != 0 { bar.ChangePct = bar.Change / pre * 100 }
        out = append(out, bar)
    }
    return out, nil
}
...
func GetIndexCurrentThs(indexCode string, wait time.Duration) (IndexCurrent, error) {
    ...
    if wait > 0 { time.Sleep(wait) }
    resp, err := client.R().Get(url)
    if err != nil { return IndexCurrent{}, err }
    text := resp.String()
    l := strings.Index(text, "{"); if l < 0 { return IndexCurrent{}, nil }
    payload := text[l:len(text)-1]
    var obj map[string]map[string]any
    if err := json.Unmarshal([]byte(payload), &obj); err != nil { return IndexCurrent{}, err }
    key := fmt.Sprintf("zs_%s", concept)
    data := obj[key]
    if data == nil { return IndexCurrent{}, nil }
