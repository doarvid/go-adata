...
func GetStockCapitalFlowBaidu(stockCode string, startDate string, endDate string, wait time.Duration) ([]FlowDaily, error) {
    ...
    client := httpc.NewClient()
    if endDate == "" { endDate = time.Now().Format("20060102") } else { endDate = strings.ReplaceAll(endDate, "-", "") }
    if startDate != "" { startDate = strings.ReplaceAll(startDate, "-", "") }
    ...
    for i := 0; i < 500; i++ {
        ...
        resp, err := client.R().Get(url)
        if err != nil { return nil, err }
        var res struct{ Result struct{ Content []map[string]string `json:"content"` } `json:"Result"` }
        if err := json.Unmarshal(resp.Body(), &res); err != nil { return nil, err }
        ...
        for _, row := range list {
            ...
            if startDate != "" && yyyymmdd < startDate { isEnd = true; break }
        ...
        if isEnd { break }
        if len(out) > 0 { endDate = strings.ReplaceAll(out[len(out)-1].TradeDate, "-", "") }
    }
    return out, nil
}
...
func parseMoney(s string) float64 {
    ...
    if len(m) < 3 { v, _ := strconvParseFloat(s); return v }
...
func strconvParseFloat(s string) (float64, error) { return strconv.ParseFloat(strings.TrimSpace(strings.ReplaceAll(s, ",", "")), 64) }
