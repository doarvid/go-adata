	...
	for i := 0; i < 2; i++ {
		...
		for page < 88 {
			...
			for _, d := range data.Data.Diff {
				idx := fmt.Sprintf("%v", d["f12"])
				name := fmt.Sprintf("%v", d["f14"])
...
func IndexConstituentBaidu(indexCode string) ([]Constituent, error) {
	...
	for page := 0; page < 100; page++ {
		...
		if data.ResultCode != "0" || len(data.Result) == 0 {
			break
		}
...
func LoadIndexCodeRelTHS() (map[string]string, error) { return IndexCodeRelTHS, nil }
